...
import { catchAsync } from "@/library/catch-async";
...
export const protect = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!accessToken) {
    return next(
      new AppError("Token not found. You are not logged in! Please log in to get access", 401),
    );
  }

  /*  INFO: 2) VERIFICATION_TOKEN */
  const decoded = jwt.verify(accessToken, Constant.instance.jwt.accessSecret) as JwtPayload;
   ...
   * */
  if (!currentUser?.email_verified && Constant.instance.server.nodeEnv !== "test") {
