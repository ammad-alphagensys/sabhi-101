...
export class Email {
  ...
  async sendConnectGoogle() {
    await this.send("connect_google_review", "Connect Your Google Account to Sabhi", {
      url: "https://platform.sabhi.io/dashboard/business-admin/review",
    });
  }

  async sendFirstPaymentLink() {
    await this.send("first_payment_link", "First Payment Link Created Successfully", {
      url: "https://platform.sabhi.io/dashboard/business-admin/messaging",
    });
  }

  async sendMessagingBasic() {
    await this.send("messaging_basic", "Get Started with Sabhi Messaging", {
      url: "https://platform.sabhi.io/dashboard/business-admin/messaging",
    });
  }

  async sendSetupAccount() {
    await this.send("setup_account", "Set Up Your Sabhi Account", {
      url: "https://platform.sabhi.io/dashboard/business-admin/sales-clerk",
    });
  }

  async sendReport() {
    await this.send("reporting", "Your Sabhi Report", {
      url: "https://platform.sabhi.io/dashboard/business-admin/report",
    });
  }
  ...
  async sendMasterMessaging() {
    await this.send(
      "master_messaging",
      "Enhance Customer Communication with Sabhi Master Messaging",
    );
  }
  ...
  async sendOneStepFromFinishing() {
    await this.send(
      "one_step_from_finishing",
      "You are One Step Away from Launching Your Business with Sabhi",
    );
  }
  // shortOnTime
  async sendShortOnTime() {
    await this.send(
      "short_on_time",
      "Short on Time? Let Sabhi Help You Launch Your Business Quickly",
    );
  }
  // threeQuickSteps
  async sendThreeQuickSteps() {
    await this.send(
      "three_quick_steps",
      "Just Three Quick Steps to Get Your Business Started with Sabhi",
    );
  }
  // trackOrder
  ...
  async sendTurnOnGoogleReviews() {
    await this.send(
      "turn_on_google_reviews",
      "Boost Your Business Reputation with Sabhi Google Reviews",
    );
  }
  // unlockSabhi
  async unlockSabhi() {
